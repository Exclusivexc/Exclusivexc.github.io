---
layout: archive
title: ""
permalink: /publications/
author_profile: true
---

{% if site.author.googlescholar %}
<div class="wordwrap">
  You can also find my articles on <a href="{{site.author.googlescholar}}"><b>Google Scholar</b></a>
</div>
{% endif %}

---
## 🎤 Conference Publications

### 🥇 **First Author Papers**

<div style="border-left: 4px solid #dc3545; padding-left: 15px; margin: 20px 0;">

<strong>SUGFW: A SAM-Based Uncertainty-Guided Feature Weighting Framework for Cold Start Active Learning</strong>  
<strong><span style="color: #dc3545;">Xiaochuan Ma</span></strong>, Jia Fu, Lanfeng Zhong, Ning zhu and Guotai Wang
<strong> Medical Image Computing and Computer Assisted Intervention (MICCAI 2025) </strong>  
[<a href="https://link.springer.com/chapter/10.1007/978-3-032-04937-7_55">Paper</a>] [<a href="https://github.com/HiLab-git/SUGFW.git">Code</a>]

</div>

<div style="border-left: 4px solid #dc3545; padding-left: 15px; margin: 20px 0;">

<strong>CLISC: Bridging Clip and Sam by Enhanced Cam for Unsupervised Brain Tumor Segmentation</strong>  
<strong><span style="color: #dc3545;">Xiaochuan Ma</span></strong>, Jia Fu, Wenjun Liao, Shichuan Zhang and Guotai wang
<strong> IEEE 22nd International Symposium on Biomedical Imaging (ISBI 2025)</strong>  
[<a href="https://ieeexplore.ieee.org/abstract/document/10980784">Paper</a>] [<a href="https://github.com/HiLab-git/CLISC.git">Code</a>]

</div>

<!-- ### 🤝 **Co-Author Papers** -->



---
## 📖 Journal Publications

<!-- ### 🥇 **First Author Papers** -->



### 🤝 **Co-Author Papers**
<div style="border-left: 4px solid #28a745; padding-left: 15px; margin: 20px 0;">

<strong>PS-seg: Learning from Partial Scribbles for 3D Multiple Abdominal Organ Segmentation </strong>
Meng Han, <strong><span style="color: #28a745;">Xiaochuan Ma</span></strong>, Xiangde Luo, Wenjun Liao, Shichuan Zhang and Guotai Wang
<strong>Neurocomputing, 2026</strong>
<em>CAS-Q2, IF: 6.5</em>  
<a href="https://www.sciencedirect.com/science/article/pii/S0925231226002341">[Paper]</a> <a href="https://github.com/HiLab-git/PartialScribble.git">[Code]</a>

</div>


<div style="border-left: 4px solid #28a745; padding-left: 15px; margin: 20px 0;">

<strong>Interpretable Medical Deep Framework by Logits-constraint Attention Guiding Graph-based Multi-scale Fusion for Alzheimer’s Disease Analysis</strong>  
Jinghao Xu, Chenxi Yuan, <strong><span style="color: #28a745;">Xiaochuan Ma</span></strong>, Huifang Shang, Xiaoshuang Shi and Xiaofeng Zhu
<strong>Pattern Recognition (PR), 2024</strong>
<em>CAS-Q1-TOP, IF: 7.6</em>  
<a href="https://www.sciencedirect.com/science/article/pii/S0031320324002012">[Paper]</a>

</div>



---



## 📝 Preprints & Under Review

<div style="border-left: 4px solid #6f42c1; padding-left: 15px; margin: 20px 0; background-color: #f8f9fa; padding: 15px; border-radius: 5px;">

<strong>MedCAL-Bench: A Comprehensive Benchmark on Cold-Start Active Learning with Foundation Models for Medical Image Analysis</strong>  
Ningzhu*, <strong><span style="color: #6f42c1;">Xiaochuan Ma*</span></strong>, Shaoting Zhang and Guotai Wang
<strong>arXiv Preprint</strong> (2025)  
[<a href="https://arxiv.org/pdf/2508.03441?">Paper</a>] [<a href="https://github.com/HiLab-git/MedCAL-Bench.git">Code</a>] 

</div>

---

## 📊 Publication Statistics

| Type | Count | Impact |
|------|-------|---------|
| 📚 **Journal Papers** | 2 | CAS Q1, Q2, Total IF: 14.1 |
| 🎤 **Conference Papers** | 2 | MICCAI, IEEE ISBI |
| 📝 **Preprints** | 1 | arXiv |
| 🥇 **First Author** | 2 | 2 Conferences |